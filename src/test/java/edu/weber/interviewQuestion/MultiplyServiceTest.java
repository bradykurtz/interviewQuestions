...
public class MultiplyServiceTest {
    ...
    public void returns_0_when_first_parameter_is_null() {
        Assert.assertEquals(0, multipleService.multiply(null, 5d), 0);
    }

    @Test
    public void returns_0_when_second_parameter_is_null() {
        Assert.assertEquals(0, multipleService.multiply(5d, null), 0);
    }
    ...
    public void five_times_five_is_25() {
        Assert.assertEquals(25, multipleService.multiply(5d, 5d), 0);
    }

    @Test
    public void five_times_zero_is_0() {
        Assert.assertEquals(0, multipleService.multiply(5d, 0d), 0);
    }

    @Test
    public void zero_times_5_is_0() {
        Assert.assertEquals(0, multipleService.multiply(0d, 5d), 0);
    }
    ...
    public void test_loop_is_efficient() {
        ...
        multipleService.multiply(5d, 100000000d);
        Instant finish = Instant.now();
        long secondParam = Duration.between(start, finish).toMillis();
        start = Instant.now();
        multipleService.multiply(100000000d, 5D);
        finish = Instant.now();
